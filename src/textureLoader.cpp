...
int TextureLoader::LoadBMP(std::string fileName, GLuint &texture)  //const char* location
{
	baseDirectory = "assets\\textures\\";
	...
	std::ifstream file(baseDirectory + fileName, std::ios::binary);
	if(!file)
	{
	...
	{
		std::cout << "File \"" << baseDirectory + fileName << "\" Not a bitmap file (wrong header)\n";
		return 2;
	}

	if (bmpInfo->biBitCount != 24)
	{
		std::cout << "File \"" << baseDirectory + fileName << "\" bits per pixel must be 24\n";
		return 2;
	}
	...
	{
		std::cout << "File \"" << baseDirectory + fileName << "\" more than one plane\n";
		return 2;
	}
...
/*******************GENERATING TEXTURES*******************/
	...
	std::cout << "Texture \"" << baseDirectory + fileName << "\" successfully loaded.\n";
